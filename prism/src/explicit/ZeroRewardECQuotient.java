...
public class ZeroRewardECQuotient extends PrismComponent
{
	...
	public void liftStrategy(int[] strat) throws PrismException
	{
		...
		for (int i = 0; i < equiv.getNumClasses(); i++) {
			...
			// compute prob1e strategy to target state *in the zero-reward fragment MDP*
			// for the states in this zero-reward EC (remain = ec, goal = target)
			BitSet prob1inEC = mc.prob1(zeroRewMDP, ec, target, false, strat);

			assert(prob1inEC.equals(ec));

			// we now have to lift the choices from the zeroRewMDP to the original MDP
			for (int s : IterableBitSet.getSetBits(ec)) {
				// skip target state
				if (target.get(s))
					continue;
