...
public:
	...
	ExportIterations(const char* title = "", const char* filename = get_export_iterations_filename()) {
		...
		fprintf(fp, "<link rel='stylesheet' href='https://automata.tools/prism-vis/iteration-vis-v1.css'>\n");
		fprintf(fp, "<script src=\"https://automata.tools/prism-vis/d3.v4.min.js\"></script>\n");
		fprintf(fp, "<body onload='init();'>\n");
		fprintf(fp, "<h1>%s</h1>\n", title);
		fprintf(fp, "<svg></svg>\n");
		fprintf(fp, "<script src=\"https://automata.tools/prism-vis/iteration-vis-v1.js\"></script>\n");
		fprintf(fp, "<script>\n");
	}
